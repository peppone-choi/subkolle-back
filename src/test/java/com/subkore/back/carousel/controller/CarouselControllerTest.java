...
class CarouselControllerTest {
    ...
    void 캐러셀을_등록할_수_있다() throws Exception {
        ...
        CarouselResponseDto carouselResponseDto =
            carouselMapper.carouselToCarouselResponseDto(carouselMapper.createCarouselRequestDtoToCarousel(createCarouselRequestDto));
        when(carouselService.createCarousel(createCarouselRequestDto)).thenReturn(carouselResponseDto);
        // when
        mockMvc.perform(post("/api/v1/carousels")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(new ObjectMapper().registerModule(new JavaTimeModule()).writeValueAsString(createCarouselRequestDto)))
                .andDo(print())
    ...
    void 캐러셀이_없을_경우_예외가_발생한다() throws Exception {
        // given
        when(carouselService.getCarouselList()).thenThrow(new CarouselException("캐러셀 아이템이 존재하지 않습니다."));
        // when
        mockMvc.perform(get("/api/v1/carousels")
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
